...
class SearchableListStateModel<ListItemType> extends ChangeNotifier {
	...
	SearchableListStateModel(
		...
		FSM.StateManager? sm = FSM.StateManager.create(
			...
			stateTransitions: [
				_dontActOnBottomSheet = FSM.StateTransition(
					name: 'dontActOnBottomSheet',
					stateChanges: {
				...
				FSM.FSMMirror(
					...
					transitions: [
						openBottomSheetMinimized = FSM.MirroredTransition(
							...
							action: (manager, additionalChanges) {
								manager.queueTransition(_dontActOnBottomSheet);
							},
						),
	...
	late final FSM.StateTransition _dontActOnBottomSheet;
