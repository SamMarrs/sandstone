...
import 'package:sandstone/state_manager.dart' as FSM;
		...
		FSM.StateManager? sm = FSM.StateManager.create(
				...
				lazyLoading = FSM.BooleanStateValue(
					...
					validateTrue: (currentState, nextState, manager) {
						return !manager.getFromState(nextState, endFound)!
							// We don't want endFound to be optimistically set to false in order to make a transition valid.
							&& !manager.getFromState(currentState, endFound)!
				...
				searching = FSM.BooleanStateValue(
					...
					validateTrue: (currentState, nextState, manager) {
						return !manager.getFromState(nextState, lazyLoading)!
							...
							// Searching resets endFound, so with this, we can optimistically have it set to false.
							&& !manager.getFromState(nextState, endFound)!;
					},
					value: false
				),
			],
			stateTransitions: [
				...
				startLazyLoading = FSM.StateTransition(
					...
					ignoreDuplicates: true,
					// If lazy loading is started optimistically, the transition action won't run.
					// This logic needs to be in a state action.
