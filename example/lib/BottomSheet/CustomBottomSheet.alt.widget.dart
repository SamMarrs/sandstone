...
enum BOTTOM_SHEET_STATES {
    MAXIMIZED,
    MINIMIZED,
	CLOSED
}
class CustomBottomSheetStateModel extends ValueNotifier<BOTTOM_SHEET_STATES> {
  CustomBottomSheetStateModel() : super(BOTTOM_SHEET_STATES.CLOSED);
	...
	GlobalKey<_CustomBottomSheetState>? _currentCustomBottomSheetState;
	...
	void showCustomBottomSheet<T>({
		...
		final gKey = GlobalKey<_CustomBottomSheetState>();
		...
		_currentBottomSheet = Scaffold.of(context).showBottomSheet<T>(
			(context) {
				_CustomBottomSheet sheet = _CustomBottomSheet(
					...
					afterMaximized: () => super.value = BOTTOM_SHEET_STATES.MAXIMIZED,
					afterMinimized: () => super.value = BOTTOM_SHEET_STATES.MINIMIZED,
					afterOpen: () => super.value = startMaximized ? BOTTOM_SHEET_STATES.MAXIMIZED : BOTTOM_SHEET_STATES.MINIMIZED,
				);
		...
		_currentBottomSheet!.closed.whenComplete(
			() {
				_currentCustomBottomSheetState = null;
				_closing = true;
				super.value = BOTTOM_SHEET_STATES.CLOSED;
			}
		);
	}
	...
	void close() {
		if (
			_currentCustomBottomSheetState != null
			&& _currentCustomBottomSheetState!.currentState != null
		) {
			_currentCustomBottomSheetState!.currentState!.close().whenComplete(() => super.value = BOTTOM_SHEET_STATES.CLOSED);
		} else {
			super.value = super.value = BOTTOM_SHEET_STATES.CLOSED;
